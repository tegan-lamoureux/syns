...
The question sheet said to not use structures from std library, so instead of using a vector here for internal storage, I am using a pointer and heap-allocated memory. I am, however, using C++11 smart pointers. This means the class can not be copied trivially with an assignment operator, but must use the std::move paradigm.
